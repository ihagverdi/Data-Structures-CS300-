...
struct cell {
	...
	cell(pair<int,int> pos = make_pair(-1,-1)) : position(pos) {
		walls = 15; // 1111 decription in binary up-right-down-left
		is_visited = false;
	};
...
void generateMaze(vector<vector<cell>> &maze) {
	...
	int num_visited = 1;
	maze[0][0].position = make_pair(0, 0);
...
void getMazeFromFile(const int& index, vector<vector<cell>> & maze) {
	...
	while (getline(inputFile, line)) {
		if (!line.empty()) {
			...
			maze[x_coord][y_coord].position = make_pair(x_coord, y_coord);
